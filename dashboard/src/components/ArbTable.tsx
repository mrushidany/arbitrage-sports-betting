  ...
  return (
    <div
      className={`rounded-xl border bg-zinc-900/60 p-4 ${
        arb.suspicious ? 'border-amber-700/60' : 'border-zinc-800'
      }`}
    >
      <div className="flex flex-wrap items-start gap-3">
        <div
          className={`rounded-lg px-2.5 py-1 text-lg font-bold tabular-nums ${
            arb.suspicious ? 'bg-amber-500/15 text-amber-400' : 'bg-emerald-500/15 text-emerald-400'
          }`}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-zinc-500">
            {arb.league} · {fmtKickoff(arb.kickoff)} · market <span className="text-zinc-400">{arb.market}</span>
          </div>
        </div>
        <div className="text-right">
          <div className="text-sm font-semibold text-emerald-400">
            +{fmtMoney(arb.guaranteedProfit)} {currency}
          </div>
          <div className="text-xs text-zinc-500">on {fmtMoney(arb.totalStake)} stake</div>
        </div>
      </div>
        ...
        {arb.legs.map((leg) => (
          <div key={leg.outcome} className="rounded-lg bg-zinc-800/50 px-3 py-2">
            <div className="flex items-center justify-between">
              ...
              <span className="tabular-nums text-sm font-semibold text-zinc-100">{leg.odds.toFixed(2)}</span>
            </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        <span className="text-xs text-zinc-600">implied {(arb.impliedSum * 100).toFixed(2)}%</span>
        <button
          onClick={handleLog}
